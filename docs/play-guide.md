...
- **In the Gallery** — Day, Outfit and Look stills open back in their own tool on their Cast (they used to open Generate). A **Cast** filter row (look-plate thumb + name) narrows to one Cast; each Cast page has **See all in Gallery**. Stills checked by Auto-review or Story's pose check carry a *pose 82% · face 64%* badge (warning-tinted on a miss), with a **Missed pose / face** filter and a **Best pose / face match** sort. **Use this pose…** (card menu) reads a still's skeleton with DWPose and saves it to the pose library or sets it as a Day slot's or Story beat's pose.
- **Cut options** — **Length** (as the shots hold · Fit to the music · 15 / 30 / 60 s) and **Cut on the beat** (with a music track), plus a **shot list**: reorder, leave out, edit captions (Day captions default to the beat), set still holds. Before encoding, Cut lists any shot Auto-review flagged or that missed its pose / face — **Retry them first**, **Leave them out** or **Cut anyway**. Story Cut works the same way.
