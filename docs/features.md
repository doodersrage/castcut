...
- **Outfit** — `/fitting` (Play workspace / Scene nav): lock a Cast character plate, **filter wardrobe kits by clothing type** (outfits, tops, outerwear, etc.), **swipe catalog wardrobe kits** (Prev / Next / chip strip with **lazy draft 512px try-on thumbs**, or full catalog select). **BYO clothing** — extract a clothing photo to a packshot, upload a ready packshot, or **Save for later** (durable browser KV, shared with Day/Story). **Auto draft previews** / **Preview kits** queues Boogu Edit Turbo (when available) at draft quality; full **Queue try-on** stays high quality. **Keep / Skip** on completed try-ons — Keep stamps a gallery keeper on the active look, maps keeper kits onto Day morning→night slots, and surfaces a primary **Continue to Day** CTA; **Compare try-ons** shows the last four results side by side. Deep links `?character=` and `?wardrobe=`; **Save kit to Cast** writes `lockedWardrobeId`; **Continue in Story** unlocks after the first Day film. Cross-links to Day and Look. Look packs apply via `?from=look`.
- **Day** — `/day` (Play bottom bar / Scene nav): pick a Cast character and plan four time slots (Morning → Night) with wardrobe kit, setting, and beat per slot; **filter outfit kits by clothing type** per session; **BYO clothing** + Image 3 **pose stick-figure** for stance unlock (optional ControlNet pose when InstantX is in Comfy inventory or Settings maps a CN weight). Queue one still or all slots. **Animate slot / Animate all** queues I2V clips from completed stills. Completed stills and clips sync into a **Day reel** (clips preferred); **Cut film** prefers server **ffmpeg** H.264/AAC MP4 via `/api/film/assemble` (720p/1080p, optional crossfade/audio bed) and falls back to browser MediaRecorder; auto-stamps when Cast is set, and surfaces first-cut **Watch on Cast** / **Open in Gallery** (`derivedKind=film`). Deep links `?character=` and `?wardrobe=` (seeds empty slot kits); Look packs via `?from=look`. Cross-links to Outfit, Look, and Story (`?from=look` when a pack is staged).
- **Look** — `/moodboard` (Play nav / Scene / ⌘K): stack up to four reference tiles (mood, lighting, location, style, palette) with optional gallery stills; choose a merge template and queue one scene still. **Extract look** builds a session look pack (vision merge when tiles have images, otherwise text cues) and **Continue to Outfit / Day / Story** hand off vibe notes + optional wardrobe lock. **Save on Cast** persists named look packs on the character record; **Export JSON** downloads a portable look pack for another machine. Deep link `?character=`; cross-links to Day, Outfit, Story, and **Film** (`/play`).
- **Film** (Play hub) — `/play` (Play workspace): guided stepper — Cast → Look → Outfit → Day → Story (optional after first cut) — with look pack + character state carried through session handoff and saved look packs on Cast. Create on Film sets name, look traits, optional **Part**, and optional **From photo** (Cast look plate). **Export / Import JSON** for look packs; **Copy share link** embeds the full pack in `/play#lookpack=…` (falls back to Export JSON when the hash exceeds URL limits). Durable **resume** restores step + saved `lookPackId` on Cast; `/play?character=&lookPack=` deep links after saving on Cast. Cutting a film in Day or Story marks the film complete and surfaces a finished state (Open film on Cast / Start new film).
- **Story** — `/story` (Play workspace, or More tools in Simple; unlocked after first Day film): **continues an existing Cast lead** (Part / From photo / look plate live on Film + Cast — Story does not re-cast). Write a bio, roll four story beats that **continue from the last pick** (unpicked cards stay off later rolls), tap a beat to queue a still or a clip in the story reel. Accepts Look packs via `?from=look&character=` (seeds **Setting**, **extra hints**, inferred **Tone**, and wardrobe lock). **Outfit for stills** (desk + phone): catalog kit or BYO packshot as Image 2; photo stills attach an Image 3 pose wireframe from beat text (intimate + social layouts), with photoreal / anti-merge locks and intimate euphemism→anatomy rewrite before queue; when InstantX is in Comfy inventory or Settings maps a ControlNet weight, the same mannequin is also sent as a pose control image on Lightning Edit. A still becomes I2V; a text-only beat is T2V; later beats continue from the last clip — **Extend clip** (Fal LTX extend-video, Grok `/v1/videos/extensions`, or Runway `/v1/video_to_video`), **Continue from last frame** (Replicate last-frame I2V; Fal soft-fails CDN upload → last-frame), or **Stitch continue** (Gemini last-frame I2V then server `/api/film/assemble` concat for the watch playlist). Story/Video surface which path ran. **Cut film** prefers server ffmpeg via `/api/film/assemble` and falls back to browser MediaRecorder; **Save to Cast** reuses that assembled blob (no second encode) when the cut succeeded. After cut: first-cut **Watch on Cast** / Gallery deep-links (manual — no auto soft-advance). **Identity for this story** From bio (default) or From photo (Cast look plate / upload). Optional **Setting** seeds a location into bios, beats, and stills. Exclusive **Tone** and **Content** rating plus an additive **Gore** toggle. Stills and clips render inline; download a zip of `story.md` + stills. Story sticks to the active Cast member (no separate session shelf). Consenting adults only; no minors. Default PG-13.
- **Cast** — `/characters`: character homes for looks, stills, clips, assembled **Films**, Part, look plate, bible, and the Story film cut. Continue reel uses Video I2V; **Continue in Story** works for Play/Outfit Cast records (synthesizes Story state from Cast bio/descriptor when needed). Roster empty CTA points at **Create on Film**. **Try on** opens Outfit; **Plan a day** opens Day; **Set look** opens Look. **LoRA flywheel** — Export → Train writes keepers under `PROMPT_DATA_DIR` and starts `/api/lora-train` with `datasetPath`; Register & pin installs into Comfy `models/loras`; **Prove it** (or auto on register) queues a validation still.
