...
- **Mobile Studio** — `/m` (Add to Home Screen via `manifest-mobile.json`): **phone-first film loop** — Capture → Queue → Rate → **Look → Outfit → Day → Story**. Touch shells on `/m/moodboard`, `/m/fitting`, `/m/day`, and `/m/play` reuse desk orchestration (swipe kits, four Day slots, Story stills **and** clips via I2V/T2V/continue). **Cut film** / **Save to Cast** are primary on phone (shared assembler, server ffmpeg preferred). Header **Desk** bridge is an optional large-screen handoff (Campaign / Look / Day / Outfit).
