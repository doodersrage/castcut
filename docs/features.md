...
- **Mobile Studio** — `/m` (Add to Home Screen via `manifest-mobile.json`): **phone-first film loop** — Cast → **Look → Outfit → Day → Story** (Queue / Gallery under More). Touch shells on `/m/moodboard`, `/m/fitting`, `/m/day`, and `/m/story` reuse desk orchestration (swipe kits, four Day slots, Story stills **and** clips via I2V/T2V/continue). **Cut film** / **Save to Cast** are primary on phone (shared assembler, server ffmpeg preferred). Header **Desk** bridge is an optional large-screen handoff to Film (`/play`).
- **Play workspace** (first-run default, **Make**) — sidebar footer / Profile → Appearance: Film, Look, Outfit, Day, Story, Gallery, and Queue. Studio is **Control**; Full is **Build**. Welcome asks **What do you want to make?** (Character / Scene·Film / Image / Surprise) before Heal & ready. Look lives under Scene nav and ⌘K (`/moodboard`). Simple remains available for a lean essentials layout.
- **Simple Essentials** — Dashboard, Generate, Cast, Story, Gallery, and Queue; Studio, Video, Refine, Compose, Inpaint, Outfit, Day, Look, and the rest sit under More tools (⌘K finds everything).
...
- **Lean gallery** — Simple and Story workspace modes use slimmer filters and bulk actions on `/gallery`.
- **Outfit** — `/fitting` (Play workspace / Scene nav): lock a Cast character plate, **filter wardrobe kits by clothing type** (quick-pick chips — outfits, tops, bottoms, outerwear, formal, swimwear — plus **More types…**), a large **Selected kit** preview, opt-in **Auto-review try-ons** (face match vs plate + vision outfit score, **Best match** marker; scores only), **swipe catalog wardrobe kits** (Prev / Next / chip strip with **lazy draft 512px try-on thumbs**, or full catalog select). **BYO clothing** — extract a clothing photo to a packshot, upload a ready packshot, or **Save for later** (durable browser KV, shared with Day/Story). **Auto draft previews** / **Preview kits** queues Boogu Edit Turbo (when available) at draft quality; full **Queue try-on** stays high quality — Queue shows a clear block reason without Cast/plate/kit/BYO/isolate. **Plate → Try-on → Keep → Day** phase strip + status line (plate · kit/BYO). **Keep / Pass / requeue** on compare cards and lightbox (Pass dismisses a try-on; Skip kit advances the deck). Keep stamps a gallery keeper on the active look, maps keeper kits onto Day morning→night slots, and surfaces a primary **Continue to Day** CTA; **Skip outfit · Day** on desk + phone. Empty clothing-type filters offer **Show all types**. Deep links `?character=` and `?wardrobe=`; **Save kit to Cast** writes `lockedWardrobeId`; **Continue in Story** unlocks after the first Day film. Cross-links to Day and Look. Look packs apply via `?from=look`. Mobile can upload / Gallery-pick a plate in-place.
- **Day** — `/day` (Play bottom bar / Scene nav): pick a Cast character and plan four time slots (Morning → Night) with wardrobe kit, setting, and beat per slot; **Suggest day** fills Setting/Beat (Queue keeps your plan); per-slot **Reroll plan**; Setting & Beat stay pinned under the board; Queue shows a clear block reason without Cast/plate/isolate. Sticky **Ready to cut** can be **Hide**’d (Cut film stays on Day reel). **Filter outfit kits by clothing type** per session; **BYO clothing** + Image 3 **pose stick-figure** for stance unlock (Image 3 Edit only — pose guides do not attach ControlNet; everyday stills default to one adult, with opt-in **Duo · companions** under the Day board). **Day mood** (pick-one group under the slot board, next to **Stills** and the **Options** switches): Everyday / Suggestive / Sport / Vacation / Intimate / Raunchy (Intimate + Raunchy require NSFW env; **Mixed / Solo / Duo** under both adult moods). Intimate/Raunchy **Duo** prefers missionary / bent / spoon / wall beats (less lap-straddle merge), stronger nude discard, and HANDS / skin-texture locks. **Suggestive** uses heat-only beats (lingerie/unzip/charged poses) that own the stance — everyday walk/coffee baselines no longer flatten the mood. **Sport** picks a random sport that fits that time of day with a mid-action pose and athletic kit lock. **Raunchy Solo** uses wardrobe-fail setup + crude self-touch punchline (not tame flash pin-ups). Slot cards show Setting · Beat (or empty hints); pose/camera/beat pools rotate for variety. **Phase + status strip** (plate · mood · stills/clips + queue block). Look→Day / Outfit Keep→Day auto-align adult Setting/Beat (anti-book). Switching Cast clears Day plates/stills so the prior look cannot stick. Queue one still or all slots. After stills, **Animate → Cut** coach; **Animate slot / Animate all** queues I2V clips from completed stills. Completed stills and clips sync into a **Day reel** (clips preferred); **Cut film** prefers server **ffmpeg** H.264/AAC MP4 via `/api/film/assemble` (720p/1080p, optional crossfade/audio bed) and falls back to browser MediaRecorder; auto-stamps when Cast is set, and surfaces first-cut **Watch on Cast** / **Open in Gallery** (`derivedKind=film`). Deep links `?character=` and `?wardrobe=` (seeds empty slot kits); Look packs via `?from=look`. Cross-links to Outfit, Look, and Story (`?from=look` when a pack is staged).
- **Look** — `/moodboard` (Play nav / Scene / ⌘K): stack up to four reference tiles (mood, lighting, location, style, palette) with optional gallery stills; choose a merge template. **Tiles → Extract → Plate → Continue** phase strip + status line (tiles · plate · pack). Extract/Queue show a clear block reason. **Preview prompt** is text only; **Queue scene** is optional (single primary Queue — result panel Queue hidden). **Extract look** builds a session look pack and soft-advances to Outfit with **Go to Day instead**. **Skip look · Day** (desk + phone). Upload / Gallery look plate in place; after a scene still, **Keep as plate** or requeue. **Continue to Outfit / Day / Story** hand off vibe notes + optional wardrobe lock. **Save on Cast** / **Export JSON** / **Share this look** under More. Mobile empty boards offer **Add starter tiles**. Deep link `?character=`; cross-links to Day, Outfit, Story, and **Film** (`/play`).
- **Film** (Play hub) — `/play` · `/m/film`: Cast → Look → Outfit → Day → Story (optional after first cut). Empty hub shows a **sample reel** + starter; Cast create is **name + From photo** first (traits under More). Mid-film has one **Continue** (Restart secondary); complete leads with Watch / Same look new Day / Story. Phone remaps every handoff to `/m/...`; Identity ready + compact persistence on the hub. **Export / Import JSON** / share link under Look packs; durable resume via `play-campaign-v1`.
- **Story** — `/story` (Play workspace, or More tools in Simple; unlocked after first Day film): **continues an existing Cast lead** (Part / From photo / look plate live on Film + Cast — Story does not re-cast). Beat picker sits above the reel with pinned Setting/Tone/Content, adult Solo/Duo/Mixed, queue block reasons, Animate→Cut coach, and lightbox requeue. **Phase + status strip** (Cast · plate · beats/clips). Write a bio, roll four story beats that **continue from the last pick** (unpicked cards stay off later rolls), tap a beat to queue a still or a clip in the story reel. Accepts Look packs via `?from=look&character=` (seeds **Setting**, **extra hints**, inferred **Tone**, and wardrobe lock). **Outfit for stills** (desk + phone): catalog kit or BYO packshot as Image 2; photo stills attach an Image 3 pose wireframe from beat text (intimate + social layouts), with photoreal / anti-merge locks and intimate euphemism→anatomy rewrite before queue; pose is Image 3 Edit only (no ControlNet on filled mannequin guides). A still becomes I2V; a text-only beat is T2V; later beats continue from the last clip — **Extend clip** (Fal LTX extend-video, Grok `/v1/videos/extensions`, or Runway `/v1/video_to_video`), **Continue from last frame** (Replicate last-frame I2V; Fal soft-fails CDN upload → last-frame), or **Stitch continue** (Gemini last-frame I2V then server `/api/film/assemble` concat for the watch playlist). Story/Video surface which path ran. **Cut film** prefers server ffmpeg via `/api/film/assemble` and falls back to browser MediaRecorder; **Save to Cast** reuses that assembled blob (no second encode) when the cut succeeded. After cut: first-cut **Watch on Cast** / Gallery deep-links (manual — no auto soft-advance). **Identity for this story** From bio (default) or From photo (Cast look plate / upload). Optional **Setting** seeds a location into bios, beats, and stills. Exclusive **Tone** and **Content** rating plus an additive **Gore** toggle. Stills and clips render inline; download a zip of `story.md` + stills. Story sticks to the active Cast member (no separate session shelf). Consenting adults only; no minors. Default PG-13.
- **Cast** — `/characters`: character homes for looks, stills, clips, assembled **Films**, Part, look plate, bible, and the Story film cut. **Status strip** (plate · Part · looks · films) warns when Outfit/Day need a plate; **Start a film** stays primary with Look/Outfit/Day/Story beside it. The home is split into tabs — **Overview** (look plate first, then looks), **Bible** (Part + bible; pasting a whole bible is folded away), **Film & media** (Film studio with encode settings under **Cut options**, plus Media), and **Packs & LoRA**; `?media=` opens Film & media and `?tab=` any tab. **Plate check** — the look plate is read with DWPose (comfyui_controlnet_aux) plus its pixel size and flagged when it has no person or face, a face turned away, more than one person, a face under ~80px wide, or a short side under 512px; off with a note when the node pack is missing. Media tiles whose clip can't load (ComfyUI offline, file gone) show *Clip unavailable* instead of a broken image. Uploading a look plate soft-advances to Outfit (cancel or Go to Day). Roster shows **No plate** readiness. Continue reel uses Video I2V; **Continue in Story** works for Play/Outfit Cast records (synthesizes Story state from Cast bio/descriptor when needed). Roster empty CTA points at **Create on Film**. Look plate **Choose from Gallery** / upload / Remove write body plate + face lock together (Remove clears every look). Phone **Watch** (`?media=films`) keeps Cast name + Day/Story/Outfit jumps. **LoRA flywheel** — Export → Train writes keepers under `PROMPT_DATA_DIR` and starts `/api/lora-train` with `datasetPath`; Register & pin installs into Comfy `models/loras`; **Prove it** (or auto on register) queues a validation still.
...
- **In-place clip playback** — WAN/LTX animated WebP and mp4/webm play on gallery cards, lightbox, Story beats, Video results, Queue, and Cast instead of a flattened still; view proxies skip Sharp flattening for motion bytes
...
- **Intimate Solo heat** — solo fingering beats use wild stances; Image 3 wrists at the pelvis; Raunchy Solo is fingers-only mid-self-touch that follows the beat stance. Nude Day stays on **Edit NSFW**; edit lead uses natural language: indoor SETTING (no beach), clothes-are-now-gone, then the beat/Image 3 pose named explicitly; Rapid **4–8 steps** CFG 1 euler_a/simple; face crop ≥1.1MP when Cast face duplicates lingerie.
- **Gallery skin refine** — Soft-pass from Gallery card / compare (default **Klein 9B Base** ReferenceLatent edit, denoise 1); keeps navel/moles while killing oily plastic. Model picker in Settings → Auto-improve. Not auto-queued after Day/Story stills.
...
- **Vision scan on still tools** — Inpaint, Outpaint, Compose (Image 1), ControlNet, and Story From photo share **Scan with vision** next to Choose from Gallery. Compose also has **Scan all images → instruction** for multi-slot transfer recipes.
...
- **Mobile Cut film** — `/m/story` and `/m/day` Cut film and Save to Cast with the same assembler as desktop (Cast / Gallery deep-links and Campaign complete CTA). Phone is first-class; Desk Continue is optional.
...
- **Quieter Settings** — Essentials ComfyUI shows engine / connection / assets / queue; maps & patching live under Show all with **Reliable / Studio / Raw** queue profiles and collapsed Expert maps; Cast-active IP-Adapter soft-hides
...
- **Dashboard output actions** — Re-queue, Refine, Story, Edit, Hints on recent stills
